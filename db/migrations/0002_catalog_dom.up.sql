-- 0002 catalog: DOM の骨格（設計書 2.4 前半）
...
CREATE TABLE catalog.asset_classes_default (
  ...
  name_ja      text NOT NULL,
  rank         smallint NOT NULL,                  -- 4=極秘 ... 1=公開
...
CREATE TABLE catalog.risk_criteria_default (
  ...
  -- 受容区分。取りうる 14 値を集合で持つ（設計書 1.5.2）
...
CREATE TABLE catalog.calendar_events_default (
  ...
  offset_months  smallint,                         -- 期首からの月数（annual/semiannual 用）
  owner_role     text NOT NULL REFERENCES catalog.roles_default(key),
  clause_ref     text,                             -- '9.2' 等
  extendable     boolean NOT NULL DEFAULT true     -- 周期延長を逸脱として許すか
);
