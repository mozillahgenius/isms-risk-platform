-- 0085 端末の登録し直し（2026-09-25）
--
-- 登録済みの機体を新しい招待で導入し直すと、devices_tenant_id_source_external_id_key に当たって
-- 登録が 400 で断られていた（2026-09-25 に実機で発生）。同じ機体なら既存の行を新しい鍵で更新する。
-- 関数の本体は 0026 の写しで、端末の行を作る部分だけを差し替える。
