-- 0086 登録済みの端末を外す（2026-09-25）
--
-- 管理者が端末の登録を外す。外した端末の報告は受けない（鍵を消すので署名を確かめられない）。
-- 再び使うには、招待からの導入し直し（登録し直し。0085）が要る。登録し直すと detached_at は消える。
-- 組織は呼び出しの文脈（app.current_tenant()）で決め、引数では受けない。

ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS detached_at timestamptz;

CREATE OR REPLACE FUNCTION app.detach_device(p_device_id uuid)
RETURNS jsonb
...
DECLARE
  ...
  v_id uuid;
BEGIN
  IF v_tenant IS NULL OR p_device_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'invalid');
  END IF;
  -- app.devices の schema_owner 向け UPDATE ポリシー（agent_device_definer_update）は
  -- WITH CHECK (tenant_id = app.agent_tenant_target()) なので、enroll_device と同じく
  -- 更新する組織をトランザクション内だけ示す。示さないと RLS で拒否される。
  PERFORM pg_catalog.set_config('app.agent_tenant', v_tenant::text, true);
  UPDATE app.devices
     SET public_key = NULL, detached_at = pg_catalog.now()
   WHERE id = p_device_id AND tenant_id = v_tenant AND source = 'agent'
  RETURNING id INTO v_id;
  PERFORM pg_catalog.set_config('app.agent_tenant', '', true);
  IF v_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'not_found');
  END IF;
  UPDATE app.agent_installations
     SET status = 'expired',
         expires_at = GREATEST(created_at + interval '1 second', pg_catalog.now()),
         failure_code = 'detached_by_admin'
   WHERE tenant_id = v_tenant AND device_id = v_id AND status = 'active';
  RETURN jsonb_build_object('ok', true, 'id', v_id);
END $$;
ALTER FUNCTION app.detach_device(uuid) OWNER TO schema_owner;
REVOKE ALL ON FUNCTION app.detach_device(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION app.detach_device(uuid) TO management_web;

-- 登録し直しで「外した」印を消す（0085 の写しに detached_at = NULL を足す）。
