-- One-way backoffice -> ISMS projection for the pilot HR identity.
-- The backoffice database remains read-only; this function writes only the
-- tenant-scoped ISMS identity/account projection.

SET ROLE schema_owner;

CREATE OR REPLACE FUNCTION app.hr_projection_target() RETURNS uuid
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog AS $$
  SELECT NULLIF(pg_catalog.current_setting('app.hr_projection_tenant', true), '')::uuid
$$;
ALTER FUNCTION app.hr_projection_target() OWNER TO schema_owner;
REVOKE ALL ON FUNCTION app.hr_projection_target() FROM PUBLIC;

CREATE POLICY hr_projection_identity_read ON app.identities
  FOR SELECT TO schema_owner USING (tenant_id = app.hr_projection_target());
CREATE POLICY hr_projection_identity_insert ON app.identities
  FOR INSERT TO schema_owner
  WITH CHECK (tenant_id = app.hr_projection_target());
CREATE POLICY hr_projection_identity_update ON app.identities
  FOR UPDATE TO schema_owner
  USING (tenant_id = app.hr_projection_target())
  WITH CHECK (tenant_id = app.hr_projection_target());

CREATE POLICY hr_projection_account_read ON app.accounts
  FOR SELECT TO schema_owner USING (tenant_id = app.hr_projection_target());
CREATE POLICY hr_projection_account_update ON app.accounts
  FOR UPDATE TO schema_owner
  USING (tenant_id = app.hr_projection_target())
  WITH CHECK (tenant_id = app.hr_projection_target());

CREATE POLICY hr_projection_device_update ON app.devices
  FOR UPDATE TO schema_owner
  USING (tenant_id = app.hr_projection_target())
  WITH CHECK (tenant_id = app.hr_projection_target());

DROP FUNCTION IF EXISTS app.project_hr_identity(uuid,text,text,text);
