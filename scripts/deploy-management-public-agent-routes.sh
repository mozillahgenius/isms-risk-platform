...
render_insert() {
  cat <<NGINX
    # Agent enrollment is authenticated by the enrollment token or signed device flow.
    # Keep the human approval UI and every other path behind GWS/OAuth.
    location = /api/agent/v1/enroll {
    ...
    location = /api/agent/v1/login-enroll/start {
        proxy_pass http://$UPSTREAM;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
    ...
    location = /api/agent/v1/login-enroll/redeem {
        proxy_pass http://$UPSTREAM;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
...
if [[ "${1:-}" == "--self-test" && $# == 1 ]]; then
  ...
  grep -Fq 'location = /api/agent/v1/enroll {' <<<"$insert"
  grep -Fq 'location = /api/agent/v1/login-enroll/start {' <<<"$insert"
  grep -Fq 'location = /api/agent/v1/login-enroll/redeem {' <<<"$insert"
  printf 'management public agent route self-test passed\n'
  exit 0
fi
...
remote_backup="$REMOTE_BACKUP_DIR/management.example.invalid.bak-agent-enrollment-$stamp"
published=0
...
if any(marker in text for marker in markers):
    raise SystemExit("management agent enrollment locations already exist")
if text.count(needle) != 1:
    raise SystemExit("expected exactly one all-other-paths marker")
block = f"""    # Agent enrollment is authenticated by the enrollment token or signed device flow.
    # Keep the human approval UI and every other path behind GWS/OAuth.
    location = /api/agent/v1/enroll {{
        proxy_pass http://{upstream};
        proxy_set_header Host $host;
...
published=0
trap - EXIT INT TERM HUP
printf '[OK] Management public agent routes deployed\n'
printf 'backup: %s\n' "$remote_backup"
