...
### 端末の登録し直し・招待の取り消し・登録を外す

- 登録済みの端末（同じ組織・同じ機体 ID）を新しい招待で導入し直すと、端末の行を新しく作らず、既存の行の公開鍵・ホスト名などを更新します（`app.enroll_device`、0085）。登録し直しにも、未使用で期限内の登録トークンが必要です。
- デバイス管理の「管理対象」から、未登録の招待を取り消せます（`app.revoke_agent_installation`、0084）。状態は `expired`、理由は `revoked_by_admin` になり、導入ページ・登録コードは期限切れとして断られます。登録済み（`active`）の招待は取り消せません。
- 登録済みの端末は「登録を外す」で外せます（`app.detach_device`、0086）。公開鍵を消すため、以後その端末の状態報告は受け付けません。紐づく招待の理由は `detached_by_admin` になり、端末の基礎情報にも出なくなります。再び使うには招待から導入し直します（登録し直すと外した印は消えます）。
