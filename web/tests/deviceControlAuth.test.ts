...
describe('端末操作の認可(fail-closed)', () => {
  it('許可リスト未設定なら誰も許可しない', () => {
    expect(isEmailAuthorizedForDeviceControl('admin@example.invalid', undefined)).toBe(false);
    expect(isEmailAuthorizedForDeviceControl('admin@example.invalid', '')).toBe(false);
  });

  it('メールが無ければ許可しない(ヘッダ未転送の経路をfail-closedにする)', () => {
    expect(isEmailAuthorizedForDeviceControl(null, 'admin@example.invalid')).toBe(false);
    expect(isEmailAuthorizedForDeviceControl('', 'admin@example.invalid')).toBe(false);
  });

  it('許可リストに一致すれば許可する(大文字小文字・前後空白を無視)', () => {
    expect(isEmailAuthorizedForDeviceControl('admin@example.invalid', ' admin@example.invalid , operator@example.invalid')).toBe(true);
  });

  it('許可リストに無いメールは拒否する', () => {
    expect(isEmailAuthorizedForDeviceControl('user@example.invalid', 'admin@example.invalid')).toBe(false);
  });
});
